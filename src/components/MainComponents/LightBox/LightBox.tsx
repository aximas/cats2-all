    ...
    return (dataArray) ? <>
        ...
        {isLoading ?
            ...
            (<div className={styles.container}>
                <button className={styles.closeBtn} aria-label="Close" onClick={() => setIsClosed(true)}
                ...
                <Splide hasTrack={false}
                        options={{rewind: true, autoHeight: false, gap: '8px', autoplay: true, type: 'loop'}}
                        aria-label={`${dataArray[0].name} images slider`} className={styles.slidersContainer}>
                    <SplideTrack className={styles.slidersTrack}>
                        {images && images.map(image => (<SplideSlide key={image.id}>
                            <img src={image.url} alt={dataArray[0].name} className={styles.sliderImg} />
                        </SplideSlide>))}
                    </SplideTrack>
                </Splide>
            </div>)}
...
export const LightBoxContainer = () => {
    ...
    return <div className={cn(styles.lightBox, {[styles.isActive]: isActive})} aria-hidden='false' role='dialog'
                tabIndex={-1}>
        <LightBox />
    </div>;
};